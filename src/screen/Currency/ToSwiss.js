...
const ToSwiss = () => {
        ...
        try {
            ...
            await axios.get(`https://${host}/latest?amount=${fromCurrencyValue}&from=${fromCurrency}&to=${toCurrency}`)
                .then(response => {
                    ...
                    if (rates.CHF != null && rates.CHF !== '') {
                        console.log('value set for CHF', rates.CHF);
                        setResultAmount(rates.CHF);
    ...
    return (
        <View >
            <Header headerText={'INR to SWISS'} />
            <Text>hello</Text>
                ...
                <SelectDropdown
                    ...
                    onSelect={(selectedItem, index) => {
                        ...
                        if (selectedItem.title === 'INR') {
                            setFromCurrency('INR');
                            setToCurrency('CHF');
                        } else if (selectedItem.title === 'CHF') {
                            setFromCurrency('CHF');
                            setToCurrency('INR');
                        }
                    ...
                    <>
                        {
                            fromCurrency === 'INR'
                                ?
                                <Text style={styles.result}>Result: Swiss {resultamount}</Text>
                                :
